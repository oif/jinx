  ...
  for (const pattern of patterns.filter((p) => p.type === "success")) {
    if (pattern.frequency >= 2) {
      insights.push({
        ...
        description: `This pattern has appeared ${pattern.frequency} times in successful evolutions.`,
  ...
  for (const pattern of patterns.filter((p) => p.type === "failure")) {
    if (pattern.frequency >= 2) {
      insights.push({
        ...
        description: `This pattern has appeared ${pattern.frequency} times in failed evolutions. Consider addressing it.`,
        evidence: pattern.examples,
        confidence: Math.min(0.5 + pattern.frequency * 0.1, 0.95),
        createdAt: now,
        reinforcementCount: 0,
